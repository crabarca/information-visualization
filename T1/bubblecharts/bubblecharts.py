...
def draw_bubblechart(canvas, data, xLabels, yLabels):
    ...
    for pais in sortedArea:
        ...
        drw.add(drw.circle(center=(x,y), r=3, fill="black", stroke_width=10, stroke="green"))
...
def get_color(data, continente):
    colors = {'America': 'rgb(222,106,89)', 'Africa': 'rgb(77,77,77)' , 'Asia':'rgb(222,177,89)' , 'Oceania':'rgb(43,135,134)' , 'Europe': 'rgb(222,140,89)'}
...
if __name__ == "__main__":
    ...
    gini = []
    hdi = []
    for pais in data.values():
        gini.append(pais['gini'])
        hdi.append(pais['hdi'])
    
    print('x axis', draw_axis_x(drw, data))
    print('y axis', draw_axis_y(drw, data))
    xLabels = draw_axis_x(drw, data)
    yLabels = draw_axis_y(drw, data)

    draw_bubblechart(drw, data, xLabels, yLabels)
